...
public class Main {
	...
	for (k = 0; k < Ants.number_of_ants; k++) {
	    Ants.ant_empty_memory(Ants.ant_colony[k]);
	}

	step = 0;
	/* Place the ants on same initial city */
	for (k = 0; k < Ants.number_of_ants; k++)
	    Ants.place_ant(Ants.ant_colony[k], step);

	while (step < Tsp.number_of_cities - 1) {
	    step++;
	    for (k = 0; k < Ants.number_of_ants; k++) {
		Ants.neighbour_choose_and_move_to_next(Ants.ant_colony[k], step);
		if (!Ants.acs_flag)
		    Ants.local_acs_pheromone_update(Ants.ant_colony[k], step);
	    }
	}

	step = Tsp.number_of_cities;
	for (k = 0; k < Ants.number_of_ants; k++) {
	    Ants.ant_colony[k].tour[Tsp.number_of_cities] = Ants.ant_colony[k].tour[0];
	    Ants.ant_colony[k].tour_length = Tsp.compute_tour_length(Ants.ant_colony[k].tour);
	    if (!Ants.acs_flag)
		Ants.local_acs_pheromone_update(Ants.ant_colony[k], step);
	}
	InOut.n_tours += Ants.number_of_ants;
    }
	...
	if (Ants.bwas_flag) {
	    Ants.trail_0 = 1. / ((double) Tsp.number_of_cities * (double) Ants.nn_tour());
	    Ants.init_pheromone_trails(Ants.trail_0);
	}
	if (Ants.mmas_flag) {
	    Ants.trail_max = 1. / ((Ants.rho) * Ants.nn_tour());
	    Ants.trail_min = Ants.trail_max / (2. * Tsp.number_of_cities);
	    Ants.init_pheromone_trails(Ants.trail_max);
	    // TODO Hier wird eine Tour mit Laenge 0 berechnet... resultiert in INFTY
	    System.out.println(Ants.nn_tour());
	}
	if (Ants.acs_flag) {
	    Ants.trail_0 = 1. / ((double) Tsp.number_of_cities * (double) Ants.nn_tour());
	    Ants.init_pheromone_trails(Ants.trail_0);
     ...
     * dependence of LocalSearch.ls_flag one of 2-InOut.opt, 2.5-InOut.opt, and 3-InOut.opt local search
     * is chosen.
     * INPUT: none
     * OUTPUT: none
     * (SIDE)EFFECTS: all ants of the colony have locally InOut.InOut.optimal tours
     ...
     * Overall best performance is typcially obtained by using 3-InOut.opt.
     */
    {
	int k;
	...
	for (k = 0; k < Ants.number_of_ants; k++) {
	    ...
	    case 1:
		LocalSearch.two_opt_first(Ants.ant_colony[k].tour); /* 2-InOut.opt local search */
		break;
	    case 2:
		LocalSearch.two_h_opt_first(Ants.ant_colony[k].tour); /* 2.5-InOut.opt local search */
		break;
	    case 3:
		LocalSearch.three_opt_first(Ants.ant_colony[k].tour); /* 3-InOut.opt local search */
		break;
	    default:
	...
	if (Ants.ant_colony[iteration_best_ant].tour_length < Ants.best_so_far_ant.tour_length) {

	    InOut.time_used = Timer.elapsed_time(); /* best sol found after InOut.time_used */
	    Ants.copy_from_to(Ants.ant_colony[iteration_best_ant], Ants.best_so_far_ant);
	    Ants.copy_from_to(Ants.ant_colony[iteration_best_ant], Ants.restart_best_ant);

	    InOut.found_best = InOut.iteration_counter;
	    InOut.restart_found_best = InOut.iteration_counter;
		...
		if (LocalSearch.ls_flag != 0) {
		    p_x = Math.exp(Math.log(0.05) / Tsp.number_of_cities);
	...
	if (Ants.ant_colony[iteration_best_ant].tour_length < Ants.restart_best_ant.tour_length) {
	    Ants.copy_from_to(Ants.ant_colony[iteration_best_ant], Ants.restart_best_ant);
	    InOut.restart_found_best = InOut.iteration_counter;
	    System.out.println("restart best: " + Ants.restart_best_ant.tour_length + " restart_InOut.found_best "
	...
	if ((InOut.iteration_counter % 100) == 0) {
	    ...
	    System.out.println("best so far " + Ants.best_so_far_ant.tour_length + ", iteration: "
		    + InOut.iteration_counter + ", time " + Timer.elapsed_time() + ", b_fac " + InOut.branching_factor);

	    if (Ants.mmas_flag && (InOut.branching_factor < InOut.branch_fac)
		    && (InOut.iteration_counter - InOut.restart_found_best > 250)) {
		/*
		...
		Ants.init_pheromone_trails(Ants.trail_max);
		Ants.compute_total_information();
		InOut.restart_iteration = InOut.iteration_counter;
		InOut.restart_time = Timer.elapsed_time();
	    }
	    System.out.println("try " + InOut.n_try + " iteration " + InOut.iteration_counter + ", b-fac "
	...
	for (k = 0; k < Ants.number_of_ants; k++)
	    Ants.global_update_pheromone(Ants.ant_colony[k]);
    }
	...
	help_b = new int[Ants.number_of_ants];
	for (k = 0; k < Ants.number_of_ants; k++)
	    help_b[k] = Ants.ant_colony[k].tour_length;
	...
	for (i = 0; i < Ants.ras_ranks - 1; i++) {
	    ...
	    Ants.global_update_pheromone_weighted(Ants.ant_colony[target], Ants.ras_ranks - i - 1);
	}
	...
	if (InOut.iteration_counter % Ants.u_gb == 0) {
	    iteration_best_ant = Ants.find_best();
	    Ants.global_update_pheromone(Ants.ant_colony[iteration_best_ant]);
	} else {
	    if (Ants.u_gb == 1 && (InOut.iteration_counter - InOut.restart_found_best > 50))
	...
	if (LocalSearch.ls_flag != 0) {
	     ...
	     */
	    if ((InOut.iteration_counter - InOut.restart_iteration) < 25)
		Ants.u_gb = 25;
	    else if ((InOut.iteration_counter - InOut.restart_iteration) < 75)
		Ants.u_gb = 5;
	    else if ((InOut.iteration_counter - InOut.restart_iteration) < 125)
		Ants.u_gb = 3;
	    else if ((InOut.iteration_counter - InOut.restart_iteration) < 250)
		Ants.u_gb = 2;
	    else
		Ants.u_gb = 1;
	} else
	...
	Ants.bwas_worst_ant_update(Ants.ant_colony[iteration_worst_ant], Ants.best_so_far_ant);
	distance_best_worst = Ants.distance_between_ants(Ants.best_so_far_ant, Ants.ant_colony[iteration_worst_ant]);
	/*
	 * System.out.println("distance_best_worst %ld, tour length worst %ld\n",distance_best_worst,ant[iteration_worst_ant
	 * ].tour_length);
	 */
	if (distance_best_worst < (int) (0.05 * (double) Tsp.number_of_cities)) {
	    Ants.restart_best_ant.tour_length = Integer.MAX_VALUE;
	    Ants.init_pheromone_trails(Ants.trail_0);
	    InOut.restart_iteration = InOut.iteration_counter;
	    InOut.restart_time = Timer.elapsed_time();
	    System.out.println("init Ants.pheromone trails with " + Ants.trail_0 + ", iteration "
		    + InOut.iteration_counter);
	} else
	 ...
	 * OUTPUT: none
	 * (SIDE)EFFECTS: none
	 * COMMENTS: this function controls the run of "InOut.max_tries" independent trials
	 */
	...
	Ants.pheromone = Utilities.generate_double_matrix(Tsp.number_of_cities, Tsp.number_of_cities);
	Ants.total = Utilities.generate_double_matrix(Tsp.number_of_cities, Tsp.number_of_cities);
